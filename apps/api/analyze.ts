...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // DB 스키마의 content_must_be_null CHECK 제약으로 이중 강제됨
    const { data: savedMessage, error: dbError } = await supabase
      .from('analyzed_messages')
      .insert({
        user_id: userId,
        content: null,          // ✅ 원문 저장 안 함 (방침 원칙 3, 1.3 ⑥단계)
    ...
    // 3. 위험 감지 시 자녀 알림 발송 (비동기 - 결과 기다리지 않음)
    // 알림 페이로드에 원문 미포함 — messageId(UUID)와 alertType만 전달
    ...
    // ✅ finally 보장: 예외 경로 포함 원문 변수 이중 파기 (방침 1.3 필수 요건)
    smsContent = null;
  }
}
